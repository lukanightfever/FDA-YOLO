...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, go=False):  # predictions, targets, model   
        ...
        if True:
            ...
            for i, pi in enumerate(p):  # layer index, layer predictions
                ...
                if n and len(tobj):
                    ...
                    adiou = adiou[0]
                    arot = arot.squeeze(1)
                    #sigloss = torch.abs(tbox[i][:,4].sigmoid()-arot.sigmoid())*3
                    w_zeros = torch.zeros_like(adiou)
                    w_ones = torch.ones_like(adiou)
                    #tobj[b, a, gj, gi] = torch.where(arot*tbox[i][:,4]>=0.0,ones, iou.detach())
                    w = torch.where(tbox[i][:,4]*arot >= 0.0, w_ones, w_zeros) 
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj
            wtobj = torch.zeros_like(pi[..., 0], device=device, dtype = torch.float32) + 0.15
            n = b.shape[0]  # number of targets
