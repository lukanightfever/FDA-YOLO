...
from utils.oriented_iou_loss import cal_diou, cal_giou, cal_iou
import cv2
import numpy as np
import pandas as pd
import torch
...
def bbox_iou(boxx1, boxx2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, device=None, same_pos=False):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    ...
    if x1y1x2y2:
        ...
        box2 = xyxy2xywh(box2)
    pi = 3.14159265358
    box1[:, 4] = box1[:, 4] * pi / 180.0
    box2[:, 4] = box2[:, 4] * pi / 180.0
    #print(box1.dtype)
    box1=box1.view(-1,box1.shape[0],box1.shape[1])
    box2=box2.view(-1,box2.shape[0],box2.shape[1])
    iou_loss, iou = cal_diou(box1,box2)
    return iou_loss,iou
'''
...
def rotation(x, y, cx, cy, tha):
    ...
    xx = (x-cx)*torch.cos(th) - (y-cy)*torch.sin(th) + cx
    yy = (x-cx)*torch.sin(th) + (y-cy)*torch.cos(th) + cy
    return xx, yy
...
def obb2poly(box):
    ...
    #print(box.shape)
    #print(xx1.shape)
    xx1 = xx1.unsqueeze(xx1.dim())
    xx2 = xx2.unsqueeze(xx2.dim())
    xx3 = xx3.unsqueeze(xx3.dim())
    xx4 = xx4.unsqueeze(xx4.dim())
    yy1 = yy1.unsqueeze(yy1.dim())
    yy2 = yy2.unsqueeze(yy2.dim())
    yy3 = yy3.unsqueeze(yy3.dim())
    yy4 = yy4.unsqueeze(yy4.dim())
    
    poly = torch.cat((xx1, yy1), 1)
    poly = torch.cat((poly, xx2), 1)
    poly = torch.cat((poly, yy2), 1)
    poly = torch.cat((poly, xx3), 1)
    poly = torch.cat((poly, yy3), 1)
    poly = torch.cat((poly, xx4), 1)
    poly = torch.cat((poly, yy4), 1)
...
def hbb_iou(boxx1, boxx2, device=None):
...
def box_iou(boxes1, boxes2, device=None):
    boxes1 = xyxy2xywh(boxes1)
    boxes2 = xyxy2xywh(boxes2)
    pi = 3.14159265358
    boxes1[:, 4] = boxes1[:, 4] * pi / 180.0
    boxes2[:, 4] = boxes2[:, 4] * pi / 180.0
    n = len(boxes1)
    m = len(boxes2)
    ious = []
    for i, boxx1 in enumerate(boxes1):
        box1 = boxx1.unsqueeze(0).repeat(m,1)
        #print(box1.shape)
        box1=box1.unsqueeze(0)
        box2=boxes2.unsqueeze(0)
        iou,_,_,_=cal_iou(box1,box2)
        iou=iou.squeeze(0)
        ious.append(iou)
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    max_det = 400  # maximum number of detections per image
