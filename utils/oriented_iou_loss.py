import torch
import numpy as np
from utils.box_intersection_2d import oriented_box_intersection_2d
from utils.min_enclosing_box import smallest_bounding_box
...
def box2corners_th(box:torch.Tensor)-> torch.Tensor:
    ...
    rotated = torch.bmm(corners.view([-1,4,2]), rot_T.view([-1,2,2])).type(torch.float32)
...
def cal_iou(box1:torch.Tensor, box2:torch.Tensor):
    ...
    #print('c2')
    #print(corners1.dtype)
    if torch.isnan(torch.sum(corners1)):
        print('cnan1')
        print(corners1)
        
    if torch.isnan(torch.sum(corners2)):
        print('cnan2')
        print(corners2)        
    ...
    area1 = box1[:, :, 2] * box1[:, :, 3]
    area2 = box2[:, :, 2] * box2[:, :, 3]
    u = area1 + area2 - inter_area + 1e-10
    iou = inter_area / u
    #if torch.isnan(torch.sum(iou)):
    #    print('iounan')
    #    print(iou)
    #    print(inter_area)
    #print(iou.dtype)
    #print(inter_area.dtype)
