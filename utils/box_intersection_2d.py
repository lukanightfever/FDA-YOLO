...
'''
import torch
from utils.cuda_op.cuda_ext import sort_v
EPSILON = 1e-7
...
def box_intersection_th(corners1:torch.Tensor, corners2:torch.Tensor):
    ...
    ''''
    if torch.isnan(torch.sum(line1_ext)):
        print('line1nan')
    if torch.isnan(torch.sum(line2_ext)):
        print('line2nan')    
    '''
    x1 = line1_ext[..., 0]
    y1 = line1_ext[..., 1]
    ...
    num = (x1-x2)*(y3-y4) - (y1-y2)*(x3-x4) +1e-6     
    den_t = (x1-x3)*(y3-y4) - (y1-y3)*(x3-x4)
    t = den_t / num
    if torch.isnan(torch.sum(den_t)):
        print(den_t)
