...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if self.augment==False:
            pp = 1
        if pp<0.0:
            img, (h0, w0), (h, w), x = load_mosaic(self, index)
        ...
        if img.shape[0]!=img.shape[1]:
            ls = max(img.shape[0], img.shape[1])
            n_img = np.zeros((ls,ls,3))
            n_img[:img.shape[0],:img.shape[1],:] = img
            img = n_img
        ...
        if self.augment:
            ...
            img = imutils.rotate(img, deg)
            x0 = img.shape[0] / 2 
            y0 = img.shape[1] / 2
            for label in labels:
                cx, cy, cw, ch, rota = label[1:]
                rota = rota - deg
                if rota > 180:
                    rota = -(360-rota)
                chwh = 0
                if abs(rota)>45:
                    if rota>0:
                        ...
                        rota = 90-abs(rota)
                    chwh = 1
                cx, cy = Srotate(deg,cx,cy,x0,y0)
                if chwh == 0:
                    label[1:] = cx, cy, cw, ch, rota
                else:
                    label[1:] = cx, cy, ch, cw, rota
                
                if cx >= 0 and cx <= w and cy >= 0 and cy <= h:
                    labeled.append(label)
            if len(labeled):
                labeled = np.stack(labeled, 0)
            labels = labeled
        if len(labels):
        ...
        #if len(labels) and self.augment:
        if False:    
            nxt = random.randint(0, len(self.img_files)-1)
            img_, (h0_, w0_), (h_, w_) = load_image(self, nxt)
            x_ = self.labels[nxt].copy()
            shape_ = self.batch_shapes[self.batch[nxt]] 
            img_, ratio_, pad_ = letterbox(img_, shape_, auto=False, scaleup=True)
            ...
            if len(x_) > 0:
                labels_ = x_.copy()
                labels_[:, 1] = ratio_[0] * w_ * (x_[:, 1] - x_[:, 3] / 2) + pad_[0]  # pad width
                labels_[:, 2] = ratio_[1] * h_ * (x_[:, 2] - x_[:, 4] / 2) + pad_[1]  # pad height
                labels_[:, 3] = ratio_[0] * w_ * (x_[:, 1] + x_[:, 3] / 2) + pad_[0]
                labels_[:, 4] = ratio_[1] * h_ * (x_[:, 2] + x_[:, 4] / 2) + pad_[1]
            
            pp = random.random()
            if pp < 0.5:
                lw = random.randint(0, w-1)
                cnt =0
                while(line_in_box(labels, labels_, lw) == 0 or lw <150 or lw > 650):
                    lw = random.randint(0, w-1)
                    cnt += 1
                    if cnt == 40:
                        pp = 0.6
                ...
                if cnt < 40:
                    img[:, lw:w0] = img_[:, lw:w0]
                    nlabel = []
                    for label in labels:
                        ex = (label[1] + label[3]) / 2
                        if ex<lw:
                            nlabel.append(label)
                    ...
                    if len(nlabel):
                        nlabel = np.stack(nlabel, 0)
                    labels = nlabel
                
            if pp >= 0.5:
                lh = random.randint(0, h-1)
                cnt =0
                while(line_in_box2(labels, labels_, lh) == 0 or lh <150 or lh > 650):
                    lh = random.randint(0, h-1)
                    cnt += 1
                    if cnt == 40:
                        break
                ...
                if cnt < 40:
                    img[lh:h0, :] = img_[lh:h0, :]
                    nlabel = []
                    for label in labels:
                        ey = (label[2] + label[4]) / 2
                        if ey<lh:
                            nlabel.append(label)
                    for label in labels_:
                        ey = (label[2] + label[4]) / 2
...
def obb_in_line(box, x):
    ...
    xx1, yy1 = rotation(x1, y1, cx, cy, tha)
    xx2, yy2 = rotation(x2, y2, cx, cy, tha)
    xx3, yy3 = rotation(x3, y3, cx, cy, tha)
    xx4, yy4 = rotation(x4, y4, cx, cy, tha)
    
    for i in range(len(xx1)):
        xmin = min(min(xx1[i], xx2[i]) , min(xx3[i], xx4[i]))
        xmax = max(max(xx1[i], xx2[i]) , max(xx3[i], xx4[i]))
        if x > xmin and x < xmax:
            return 0
    return 1

def line_in_box(label1, label2, x):
    box1 = label1[:,1:].copy()
    box2 = label2[:,1:].copy()
    return obb_in_line(box1, x) & obb_in_line(box2, x)

def obb_in_line2(box, y):
    ...
    for i in range(len(xx1)):
        ymin = min(min(yy1[i], yy2[i]) , min(yy3[i], yy4[i]))
        ymax = max(max(yy1[i], yy2[i]) , max(yy3[i], yy4[i]))
        if y > ymin and y < ymax:
            return 0
    return 1

def line_in_box2(label1, label2, y):
    box1 = label1[:,1:].copy()
    #print(label1)
    #print(label2)
    box2 = label2[:,1:].copy()
    return obb_in_line2(box1, y) & obb_in_line2(box2, y)
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        #tid = int(path.split('\\')[-1][:-4])
        #print(index, self.id2index[tid])
        #print(tid)
...
def load_mosaic(self, index):
    ...
    if img is None:
        ...
        gid = int(path.split('/')[-1][:-4])
        cx, cy = self.table.find_line(gid)
        ig1, ig2, ig3 = self.table.find_g(cx, cy)
        ig1 = int(self.id2index[ig1])
        ig2 = int(self.id2index[ig2])
        ig3 = int(self.id2index[ig3])
        
        p1 = self.img_files[ig1]
        p2 = self.img_files[ig2]
        p3 = self.img_files[ig3]
        ...
        x0 = self.labels[index].copy()
        x1 = self.labels[ig1].copy()
        x2 = self.labels[ig2].copy()
        x3 = self.labels[ig3].copy()
        ...
        if len(x):
            x = np.stack(x,0)
        #print(x1.shape)
    return imgf, (h0, w0), imgf.shape[:2], x
