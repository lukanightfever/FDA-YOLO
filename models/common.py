...
class SE1(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.sig = nn.Sigmoid()
        self.round_grid = round_()
        self.round_grid.concatt()
        print('grid_finish')
    def forward(self, x):
        #print(x.shape)
        if x.shape[2] < 128:
        ...
        #print(torch.sum(proj_value))
        proj_key = proj_key.unsqueeze(4)
        proj_key = proj_key.expand(proj_key.shape[0],proj_key.shape[1],proj_key.shape[2],proj_key.shape[3],9)
        
        proj_query = proj_query.unsqueeze(4)
        proj_query = proj_query.expand(proj_query.shape[0],proj_query.shape[1],proj_query.shape[2],proj_query.shape[3],9)
        proj_value = proj_value.unsqueeze(4)
        proj_value = proj_value.expand(proj_value.shape[0],proj_value.shape[1],proj_value.shape[2],proj_value.shape[3],9)
        
        proj_key = proj_key.permute(0,4,2,3,1)
        proj_query = proj_query.permute(0,4,2,3,1)
        proj_value = proj_value.permute(0,4,2,3,1)
         
        #print('1')
        for batch in range(proj_key.shape[0]):
            keys = proj_key[batch]
            for c1 in range(keys.shape[0]-1):
                keys[c1+1] = keys[0][self.round_grid.sp[c1][:,:,0],self.round_grid.sp[c1][:,:,1]]
        
        for batch in range(proj_value.shape[0]):
            keys = proj_value[batch]
            for c1 in range(keys.shape[0]-1):
                keys[c1+1] = keys[0][self.round_grid.sp[c1][:,:,0],self.round_grid.sp[c1][:,:,1]]
        
        attn = proj_key * proj_query
        attn = torch.exp(torch.sum(attn, dim = 4))
        attn = attn / torch.sum(attn,dim = 1).unsqueeze(1)
        attn = attn.unsqueeze(-1)        
        
        proj_value = torch.sum(proj_value * attn, dim = 1) 
        proj_value = proj_value.permute(0,3,1,2) 
        #print(self.gamma)
        
        return self.sig(proj_value) + x    
...
class SE1(nn.Module):
    def __init__(self, in_dim, ratio=16, kernel_size=7):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(SE1, self).__init__()
        c1 = in_dim
